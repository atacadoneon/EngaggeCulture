...
const CATALOGO_DEMO = [
  { id: "1", nome: "Placa Top Performance 20x25cm", descricao: "Placa em acrilico 3D com impressao UV personalizada. Acabamento premium com alto relevo. Personalizacao com nome, conquista e logo da empresa.", imagem: "https://images.unsplash.com/photo-1567427017947-545c5f8d16ad?w=400&h=400&fit=crop", preco: 149.90, preco_promocional: null, categoria: "placa", estoque: 50, avaliacao: 4.9, vendidos: 234, sku: "PLC-001" },
  { id: "2", nome: "Trofeu Campeao de Vendas 25cm", descricao: "Trofeu em acrilico 3D com base em madeira nobre. Design moderno e elegante. Gravacao a laser personalizada.", imagem: "https://images.unsplash.com/photo-1569429593410-b498b3fb3387?w=400&h=400&fit=crop", preco: 289.90, preco_promocional: 249.90, categoria: "trofeu", estoque: 20, avaliacao: 4.8, vendidos: 87, sku: "TRF-001" },
  { id: "3", nome: "Kit Onboarding Essencial", descricao: "Kit de boas-vindas com placa welcome personalizada + carta do CEO. Embalagem premium para impressionar no primeiro dia.", imagem: "https://images.unsplash.com/photo-1513128034602-7814ccaddd4e?w=400&h=400&fit=crop", preco: 89.90, preco_promocional: null, categoria: "kit", estoque: 100, avaliacao: 5.0, vendidos: 456, sku: "KIT-001" },
  { id: "4", nome: "Kit Onboarding Premium", descricao: "Kit completo: placa welcome + caneca personalizada + camiseta + adesivos + carta do CEO. A experiencia completa de boas-vindas.", imagem: "https://images.unsplash.com/photo-1586880244406-556ebe35f282?w=400&h=400&fit=crop", preco: 199.90, preco_promocional: 179.90, categoria: "kit", estoque: 35, avaliacao: 4.9, vendidos: 178, sku: "KIT-002" },
  { id: "5", nome: "Medalha de Conquista Dourada", descricao: "Medalha em metal com fita personalizada. Ideal para campanhas de vendas, eventos e premiacoes rapidas. Embalagem individual.", imagem: "https://images.unsplash.com/photo-1579783902614-a3fb3927b6a5?w=400&h=400&fit=crop", preco: 39.90, preco_promocional: null, categoria: "medalha", estoque: 200, avaliacao: 4.7, vendidos: 890, sku: "MDL-001" },
  { id: "6", nome: "Placa Tempo de Casa 25x30cm", descricao: "Placa em acrilico com moldura em madeira. Para premiar marcos de 1, 3, 5, 10 anos. Personalizacao completa.", imagem: "https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=400&h=400&fit=crop", preco: 189.90, preco_promocional: null, categoria: "placa", estoque: 40, avaliacao: 4.8, vendidos: 156, sku: "PLC-002" },
  { id: "7", nome: "Quadro de Valores da Empresa", descricao: "Painel em acrilico com impressao UV dos valores da empresa. Moldura moderna. Tamanho 50x70cm para ambientacao.", imagem: "https://images.unsplash.com/photo-1497366216548-37526070297c?w=400&h=400&fit=crop", preco: 249.90, preco_promocional: null, categoria: "quadro", estoque: 15, avaliacao: 4.6, vendidos: 34, sku: "QDR-001" },
  { id: "8", nome: "Caneca Personalizada Premium", descricao: "Caneca de ceramica de alta qualidade com personalizacao full color. Nome do colaborador + logo da empresa. Embalagem individual.", imagem: "https://images.unsplash.com/photo-1514228742587-6b1558fcca3d?w=400&h=400&fit=crop", preco: 34.90, preco_promocional: 29.90, categoria: "outro", estoque: 300, avaliacao: 4.5, vendidos: 1230, sku: "CNC-001" },
];
...
export default function PaginaLojaInterna() {
  ...
  function adicionarAoCarrinho(produto: typeof CATALOGO_DEMO[0], qtd: number) {
  ...
  function adicionarEFecharModal() {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Link href="/loja-interna/pedidos">
            <Button variante="secundario"><Package className="h-4 w-4" />Meus Pedidos</Button>
          </Link>
          <Link href="/loja-interna/carrinho">
            <Button className="relative">
              <ShoppingCart className="h-4 w-4" />
              Carrinho
              {totalCarrinho > 0 && (
                <span className="absolute -top-2 -right-2 h-5 w-5 bg-rose-500 rounded-full text-[10px] font-bold text-white flex items-center justify-center">
                  {totalCarrinho}
                </span>
              )}
            </Button>
      ...
      {totalCarrinho > 0 && (
        <div className="flex items-center gap-3 bg-violet-500/10 border border-violet-500/20 rounded-xl px-4 py-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-violet-400">{totalCarrinho} {totalCarrinho === 1 ? "item" : "itens"} no carrinho</p>
            <p className="text-xs text-zinc-500">Total: R$ {valorCarrinho.toFixed(2)}</p>
          </div>
          <Link href="/loja-interna/carrinho">
            <Button tamanho="sm">Ver Carrinho</Button>
          </Link>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        {filtrados.map((produto) => {
          ...
          const noCarrinho = carrinho.itens.find((i) => i.id === produto.id);
          const acabouDeAdicionar = adicionadoId === produto.id;
            ...
            <div key={produto.id} className="group bg-zinc-900 rounded-2xl border border-zinc-800 overflow-hidden hover:border-violet-500/40 hover:shadow-xl hover:shadow-violet-500/5 transition-all duration-300">
              <div className="relative h-48 bg-zinc-800 overflow-hidden">
                ...
                <div className="absolute top-3 left-3 flex flex-col gap-1.5">
                  {temPromocao && <span className="text-[10px] font-bold bg-rose-600 text-white px-2 py-0.5 rounded">-{desconto}% OFF</span>}
                  {produto.estoque <= 20 && <span className="text-[10px] font-bold bg-amber-600 text-white px-2 py-0.5 rounded">Ultimas unidades</span>}
                </div>
              ...
              <div className="p-4">
                ...
                <div className="flex items-center gap-3 mt-2">
                  <div className="flex items-center gap-1"><Star className="h-3.5 w-3.5 text-amber-400 fill-amber-400" /><span className="text-xs font-semibold text-white">{produto.avaliacao}</span></div>
                ...
                <div className="flex items-center justify-between mt-4 pt-3 border-t border-zinc-800">
                  ...
                  {acabouDeAdicionar ? (
                    <span className="flex items-center gap-1 text-xs font-semibold text-emerald-400 animate-in fade-in">
                      <CheckCircle className="h-4 w-4" /> Adicionado!
                    </span>
                  ) : noCarrinho ? (
                    <div className="flex items-center gap-1">
                      <Badge cor="violet">{noCarrinho.quantidade}x</Badge>
                      <Button tamanho="sm" variante="secundario" onClick={() => adicionarAoCarrinho(produto, 1)}>
                        <Plus className="h-3 w-3" />
                      </Button>
                    </div>
                  ) : (
                    <Button tamanho="sm" onClick={() => adicionarAoCarrinho(produto, 1)}>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <div className="flex items-center gap-1"><Star className="h-4 w-4 text-amber-400 fill-amber-400" /><span className="text-sm font-bold text-white">{produtoSelecionado.avaliacao}</span></div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-xs text-zinc-400"><Check className="h-3.5 w-3.5 text-emerald-400" />Personalizacao com logo e dados da sua empresa</div>
                  <div className="flex items-center gap-2 text-xs text-zinc-400"><Check className="h-3.5 w-3.5 text-emerald-400" />Producao sob demanda em 15 dias uteis</div>
                  <div className="flex items-center gap-2 text-xs text-zinc-400"><Check className="h-3.5 w-3.5 text-emerald-400" />Entra direto no seu estoque de fulfillment</div>
                </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-zinc-800">
                  ...
                  <Button tamanho="lg" onClick={adicionarEFecharModal}>
